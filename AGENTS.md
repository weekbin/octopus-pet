...
- **v5.2 → v10.6 演进根因 (2026-09-10 治本 H3 源破渲染绿)**: v5.2 (BiRefNet+CorridorKey 物理级 unmixing) 治本 12× 绿残留下降后, **剩余 5% 绿来自 H3 源视频在 prop 进出场景的"破渲染"** — H3 diffusion 在 prop 淡入/淡出帧把 prop 渲成绿色 (detective f67-72 帽子+放大镜, worker f47-58 桌子+头盔+工具+锤, drink-coffee 完美无问题). 5 步演进治本:
  - **v10.1 per-pixel gate (回归!)**: `a>128 & G>R+15 & G>B+15 & G>80` — 抓 H3 破渲染绿 OK, 但 f67-71 detective 误伤暗橄榄 (RGB 63,101,33, G/(R+B+1)=1.04, 1.04<1.15 救不了), LOST 689→5/1966/2085/2208 (大赢) 但 f67-72 H3 暗色 prop 误判 demote, detective 帽子"破洞" + worker f48-58 H3 破渲染绿饱和度高反而 demote 更多 (LOST 825→2061).
  - **v10.2 ratio gate (替代 v10.1)**: `G/(R+B+1) > 1.15 & G>80` (a>128) — 修复 f67-71 (LOST 689→5 大赢), 但 worker f48/50/56-58 反向退化 (worker's 破渲染绿饱和度跨帧漂移, ratio 门阈值 1.15 卡中间).
  - **v10.3 strict gate (最终颜色规则)**: `a>128 & G>150 & R<150 & B<150 & ratio>1.3` — 只抓明显亮绿. detective f67-70 修到 LOST 4/5/9/704 (vs v5.4 135/608/886/1106), worker f52-55/59 改善, f48/50/56-58 仍退化. **v10.3 BUG**: 第一次跑 v10.3 时 `corrkey_unmix(corrkey, bgr, ...)` 把 BGR 当 RGB 传, unmixing 颜色全反, drink-coffee 整图染蓝 (mean RGB 90,109,208). 修一行 `rgb` not `bgr`, drink 恢复 warm (212,97,87).
  - **v10.4 inpaint 方案 B (部分有效)**: 检测 green-in-fg 像素 → cv2.distanceTransformWithLabels 找最近非绿 fg 像素 → 借其 RGB. 实际只改 60-300 像素/帧 (max diff 170), 因为整个 prop 区域都绿调, 借到的"非绿 fg" 也在同区域 (是身体色不是 prop 真实色). **不够, 放弃**. 教训: "找最近非绿" 在饱和绿区域失效, 需要外部干净 prop 色源.
  - **v10.5 borrow-only 方案 C (fill hole)**: 取 f64 (detective) / f46 (worker) 作参考帧 → phaseCorrelate body 求 sub-pixel shift (max ±8px) → cv2.warpAffine warp 参考 → 对 (current_a<128 & warped_a>128) 像素 fill warped. detective f70 LOST 704→1, f71 1668→5, f72 1849→19; worker f48 2655→201. **fill hole 大赢, 但不解决现有 green-in-fg 像素 (它们 alpha>128, 不在 fill mask)**.
  - **v10.6 borrow+recolor 方案 C+B (CURRENT BEST)**: v10.5 fill 基础上, **加 recolor step**: 对 (green-in-fg 当前 & warped_a>128 & warped 非绿) 像素, 用 warped RGB 替换 current RGB (alpha 保持). 双重修复: 1) 借参考帧的真 prop 色改写当前绿调, 2) 借参考帧的 prop shape 填补 hole. **detective f68-72 绿残 692/1973/1381/505/261 → 84/126/112/100/54 (-79% ~ -94%)** ✅; worker f48-58 LOST -92% ~ -98% (绿色残留本来就低, recolor 效果小, fill 大赢). **drink-coffee 三版完全一致** (sanity check 通过, recolor mask 0 命中).
  - **算法细节**: phaseCorrelate 响应 < 0.05 时 fallback 到 identity shift (避免错误 warp); `not_black = warped.sum>30` 排除 alpha=0 像素 (避免把背景黑色当 prop 借); green-in-fg mask 用 `G>R+8 & G>B+8 & G>60` (loose, 抓所有绿调); recolor mask 额外要求 `warped non-green` (避免把绿色"反借"到当前帧).
  - **教训**: 1) H3 diffusion 模型的"prop 淡入淡出绿"是模型物理光照模拟, prompt 几乎治不了 (已经在 prompt 加了 8 段约束, 模型仍渲绿). 2) "借参考帧 + warp + recolor" 是物理级修复, 治本率 80-95% (剩余 5% 是 prop 形状跟参考帧差异大, warp 对不齐). 3) worker 的 prop (桌子在脚下) 跟 detective 的 prop (帽在头顶) 几何差异大, 需要**不同参考帧策略** (worker f47-50 借 f46, f56-58 借 f60 — prop 消失后帧) 才能更准.
  - **下一步**: v10.7 worker 双参考帧 (f46 供 prop-entering frames, f60 供 prop-exiting frames); 改 H3 worker prompt 加"桌子淡出时必须保持棕色, 不反射绿" 强制约束.
