...
| 项 | 值 |
|---|---|
| 状态 | **V3.0 (2026-09-16) P0 解除**: H3 视频生成 ✅ / 验证 ✅ (18 合格) / mp4 rsync ✅ / v10-final 重生成 18 APNG ✅ (BiRefNet+CorridorKey 治本白方块) / flicker post-fix **v7 four-pass** ✅ (Pass 4 global majority voting 70% threshold 治本残留 body 透明像素; Pass 2 改用 alpha<100>28000 硬阈值 + full silhouette 模板替换治本半脸; 5 场景 44 bad-frames 100% 清零 + 85723 majority-voting 像素修复; 26 场景全套跑过; 32-laugh f030/f042/f044 大笑姿势一致无半脸) / scenes.json 8→26 ✅ / check-scenes-sync + lint + test ✅. 待 `release-plugin.sh` 跑 V3.0 release. 接手任务见 `HANDOFF.md` |
...
- **v10-final 必跑 post-fix flicker 修复 v7 four-pass** (2026-09-16 加): `scripts/extract-v10-final-postfix-flicker.py`. v10-final 在含大量飞舞道具的场景 (17-celebrate 彩带 / 22-yay-friday 酒杯 / 23-dancing 音符 / 32-laugh 哈哈字 / 36-cheer 彩旗) 上四类 alpha mask 不稳定: (a) 1-frame flicker (像素级抖动) / (b) 整章鱼空白帧 (CorridorKey 跨帧 unmixing 完全失败, alpha_mean<10 连续数帧) / (c) sub-silhouette 位置性 uncertain (v10-final 在某些像素位置 consistently 给低 alpha, 不是 flicker 是模型不确定, Pass 3 median 抓不到) / (d) **v7 新覆盖**: 稳定身体像素 consistently 给低 alpha (不是 flicker, 模型整个位置性 uncertain). disposal=2 下四类都视觉闪. **Pass 1** — per-pixel temporal fill, ±7 滚动窗口 max alpha≥200, 像素 p: curr_a<150 + RGB sum>300 → fill alpha=255. **Pass 2** — bad-frame template replace, 帧 alpha<100 像素数 > 28000 (v7 改用硬阈值, v6 用 alpha_mean<60 被 Pass 1 修边抬升后逃过) 视为完全空白 → 找 ±15 范围最近模板帧 (alpha_mean>80) → **直接模板替换整个 silhouette** (full_sil = template_a>200, RGB + alpha 同步覆盖, v6 用 sil_mask 让 Pass 1 修过的边缘像素不在 mask 内 → silhouette 边缘 alpha 不对称 → 半脸, 32-laugh f030 验证). **Pass 3** — sub-silhouette temporal median filter, ±5 帧 alpha **中位数** ≥200 → fill, 跳过 Pass 2 命中帧避免半脸. median 比 max 更鲁棒 (≥3/11 帧不透明就修). **Pass 4 NEW** — global per-pixel majority voting: 跨全部 99 帧计算每像素 alpha>=200 帧数 ≥70% → stable_mask (跨 26 场景 stable body 像素 11000-13000). fill stable_mask 内 alpha<200 + RGB sum>300 像素到 alpha=255. RGB sum>300 守卫避免 fill 腐蚀成黑色背景. 70% 阈值平衡: 60% fill 姿势切换帧 (32-laugh 大笑爆发嘴张开/合拢时嘴内/嘴外切换), 80% 覆盖不足. 算法 idempotent. 5 场景共 333849 flicker 像素修复 + 44 坏帧模板替换 + 71133 sub-silhouette 像素修复 + 13749 majority-voting 像素修复; 26 场景全套 85723 majority-voting 像素. **任何走 v10-final 输出的 APNG 上桌前必跑 flicker post-fix v7**.
