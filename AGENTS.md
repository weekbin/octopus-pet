...
| 项 | 值 |
|---|---|
| 状态 | **V3.0 (2026-09-16) P0 解除**: H3 视频生成 ✅ / 验证 ✅ (18 合格) / mp4 rsync ✅ / v10-final 重生成 18 APNG ✅ (BiRefNet+CorridorKey 治本白方块) / flicker post-fix v6 three-pass ✅ (5 场景 44 bad-frames 100% 清零 + 71133 sub-silhouette 像素修复, 17-celebrate 17 / 22-yay-friday 5 / 23-dancing 2 / 32-laugh 17 / 36-cheer 3) / scenes.json 8→26 ✅ / check-scenes-sync + lint + test ✅. 待 `release-plugin.sh` 跑 V3.0 release. 接手任务见 `HANDOFF.md` |
...
- **v10-final 必跑 post-fix flicker 修复 v6 three-pass** (2026-09-16 加): `scripts/extract-v10-final-postfix-flicker.py`. v10-final 在含大量飞舞道具的场景 (17-celebrate 彩带 / 22-yay-friday 酒杯 / 23-dancing 音符 / 32-laugh 哈哈字 / 36-cheer 彩旗) 上三类 alpha mask 不稳定: (a) 1-frame flicker (像素级抖动) / (b) 整章鱼空白帧 (CorridorKey 跨帧 unmixing 完全失败) / (c) sub-silhouette 位置性 uncertain (v10-final 在某些像素位置 consistently 给低 alpha, 不是 flicker 是模型不确定). disposal=2 下三类都视觉闪. **Pass 1** — per-pixel temporal fill, ±7 滚动窗口 max alpha≥200, 像素 p: curr_a<150 + RGB sum>300 → fill alpha=255. **Pass 2** — bad-frame template replace, 帧 alpha_mean<60 (v6 放宽, v5 是<30 覆盖半透明章鱼帧如 32-laugh f42) 视为异常 → 找 ±15 范围最近模板帧 (alpha_mean>80) → 复制模板 silhouette (alpha>200, v6 放宽到整个 silhouette) 的 RGB+alpha. RGB 必须一起复制. **Pass 3** — sub-silhouette temporal median filter, ±5 帧 alpha **中位数** ≥200 → fill, 跳过 Pass 2 命中帧避免半脸. median 比 max 更鲁棒 (≥3/11 帧不透明就修). 算法 idempotent. 5 场景共 333849 flicker 像素修复 + 44 坏帧模板替换 + 71133 sub-silhouette 像素修复. **任何走 v10-final 输出的 APNG 上桌前必跑 flicker post-fix v6**.
