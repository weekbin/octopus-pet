...
| 项 | 值 |
|---|---|
| 状态 | **V3.0 (2026-09-16) P0 解除**: H3 视频生成 ✅ / 验证 ✅ (18 合格) / mp4 rsync ✅ / v10-final 重生成 18 APNG ✅ (BiRefNet+CorridorKey 治本白方块) / flicker post-fix v5 two-pass ✅ (5 场景 44 bad-frames 100% 清零, 17-celebrate 17 / 22-yay-friday 5 / 23-dancing 2 / 32-laugh 17 / 36-cheer 3) / scenes.json 8→26 ✅ / check-scenes-sync + lint + test ✅. 待 `release-plugin.sh` 跑 V3.0 release. 接手任务见 `HANDOFF.md` |
...
- **v10-final 必跑 post-fix flicker 修复 v5 two-pass** (2026-09-16 加): `scripts/extract-v10-final-postfix-flicker.py`. v10-final 在含大量飞舞道具的场景 (17-celebrate 彩带 / 22-yay-friday 酒杯 / 23-dancing 音符 / 32-laugh 哈哈字 / 36-cheer 彩旗) 上两类 alpha mask 不稳定: (a) 1-frame flicker (像素级抖动) / (b) 整章鱼空白帧 (CorridorKey 跨帧 unmixing 完全失败, 17-celebrate 8 帧连续 alpha=0 / 32-laugh f29-f33 5 帧空白). disposal=2 下两类都视觉闪. **Pass 1** — per-pixel temporal fill, ±7 滚动窗口, 像素 p: curr_a<150 + RGB sum>300 + 近邻 max alpha≥200 → fill alpha=255. **Pass 2** — bad-frame template replace, 帧 f_i alpha_mean<30 视为异常 → 找 ±15 范围最近模板帧 (alpha_mean>80 = 章鱼完整) → 复制模板 silhouette (alpha>100) 的 RGB+alpha. RGB 必须一起复制 (空白帧 silhouette 像素 RGB 被腐蚀成黑色, 只复制 alpha 出来章鱼变暗红). 算法 idempotent. 5 场景共 333849 flicker 像素修复 + 42 坏帧模板替换, 44 frames bad-frames 100% 清零. **任何走 v10-final 输出的 APNG 上桌前必跑 flicker post-fix v5**.
