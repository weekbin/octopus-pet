...
| 失败记录 (2026-09-16 18:55) | **方案 G (`ffmpeg tmix=frames=3:weights=1 1 1`) 实证失败已回滚** — 用户反馈 17/22/23/32 残影感, 试 tmix 3 帧 temporal mix. 实证: 32-laugh 41 raw 帧 α=0 (BLANK) + 17-celebrate 22 + 22-yay-friday 8 + 23-dancing 1, 即便 Pass 2 fill 救回半截 (5681-6348 opaque vs v9 v2 的 13356-13876), 视觉深红/泪印 (aRGB=(127,6,6,α=1)). **完整回滚**: 4 APNG + 4 mp4 sha256 100% 匹配 backup. 当前 26 场景**全部 v9 v2 状态**. tmix 在大笑高潮帧叠加成"无脸" → CorridorKey α=0 → 即便 fill, RGB 用前一帧 (同段笑, 边缘绿幕反射污染) → 视觉恶化. 续战方向 (G1 tmix=frames=2 保守 / G2 mpdecimate + 12fps / G3 接受 v9 v2 + 调透明度阴影缓解感). 详 `CHANGELOG.md` [Unreleased] "Out of scope" + `HANDOFF.md` §5.7 |
| 方案 G1 (V3.0.1, 2026-09-16 19:13 → 19:24 撤回) | **方案 G1 (`ffmpeg tmix=frames=2:weights=1 1`) 像素统计看似 OK, 视觉复查同样有拖影 + 绿幕残留, 已撤回** — 用户在打包后立即反馈"拖影比较严重, 还出现了绿幕没处理干净的情况. 在没处理好 apng 的效果之前不要忙着打包". 全 99 帧 contact sheet 检查发现 G1 (及 v9 v2 baseline) 在大笑爆发/彩带挥舞段都有 dark green spike artifacts (CorridorKey 对运动模糊帧提取不全): 32-laugh f15-f54, 17-celebrate f38-f43, 22-yay-friday f37-f38, 23-dancing f57-f59. **回滚**: 4 APNG + 4 mp4 sha256 100% 恢复 v9 v2 baseline. **V3.0.1 release 已删除** (`gh release delete v3.0.1`), V3.0 恢复为 Latest. **当前 26 场景 = v9 v2 baseline**. tmix 是错的解法: 它不能治本 H3 源视频 motion blur + CorridorKey 边缘 artifact. 下一版另起方案. 详 `CHANGELOG.md` [Unreleased] "方案 G1 视觉复查失败" + `HANDOFF.md` §5.8 |
| 方案 H1 (V3.0.1 新准备, 2026-09-16 19:56) | **方案 H1 (v10-final 加 Stage 3.5 `post_faint_decoration_cleanup_h1`) 实证成功** — 用户接续由简到难方向. 加 `alpha 5-100 且 RGB 非体色粉 → α=0` Pass 清理道具装饰 partial alpha 拖影. 4 场景全跑: 32-laugh partial alpha 5-100 像素 163175→16403 (-90%), 17-celebrate 105096→7855 (-93%), 22-yay-friday 134669→13739 (-90%), 23-dancing 111734→6049 (-95%); opaque 像素 0 变化 (体色保护). 全 99 帧 contact sheet 视觉复查: 32-laugh f15-f54 上方 dark spike artifacts 几乎完全消失, 17/22 装饰保留, 23 同样干净. 详 `CHANGELOG.md` [Unreleased] "Added" + `HANDOFF.md` §5.7 |
| 方案 H2 (V3.0.1 准备, 2026-09-16) | **方案 H2 (postfix-flicker 加 Pass 5 跨帧稳定度阈值清装饰残影) 实证成功** — 用户视觉复查 H1 反馈"还有点闪烁 + 部分身体帧帧消失", 逐帧诊断根因: H1 治了 alpha 5-100 (90-95%), 但残留 alpha 100-200 装饰色像素 (彩带/音符/酒杯 partial alpha 边缘) 在 silhouette 内, silhouette mask 无法区分 (装饰紧贴 body). **新算法 Pass 5**: 跨 99 帧 alpha>=100 帧数比例 = 稳定度, 阈值 0.7; body 主体稳定度 >=0.7 保留, 装饰像素稳定度 <0.7 + alpha 100-200 + 非体色粉 → α=0. 4 场景全跑 pass5=18847/19357/26298/16717 decoration residues (与离线模拟完全一致); 0 误伤 body opaque + 0 误伤 body AA. **4 类分离验证** (H2 离线 99 帧累加): A body_stable_opaque 1.1-1.3M + B body_aa_edge 100k-171k + C decoration_core 478k-535k 全部保留; D decoration_partial 16k-26k (1.0-1.4%) 全清. 全 99 帧 contact sheet 视觉复查: 17/22/23 装饰核心保留 (彩带/音符/酒杯) + alpha 100-200 装饰色残影显著减少, 32-laugh f15-f54 几乎干净. **当前 4 场景 v9 v5 = H2 + 22 场景 v9 v2 = 26 场景 V3.0.1 baseline 待发布** (用户硬约束 "不要急着打包"). 详 `CHANGELOG.md` [Unreleased] "Added" + `HANDOFF.md` §5.9 |
...
- **v10-final 必跑 post-fix flicker 修复 v9 v2 four-pass** (2026-09-16 加): `scripts/extract-v10-final-postfix-flicker.py`. v10-final 在含大量飞舞道具的场景 (17-celebrate 彩带 / 22-yay-friday 酒杯 / 23-dancing 音符 / 32-laugh 哈哈字 / 36-cheer 彩旗) 上四类 alpha mask 不稳定: (a) 1-frame flicker / (b) 整章鱼空白帧 (CorridorKey 跨帧 unmixing 完全失败) / (c) sub-silhouette 位置性 uncertain / (d) **v9 v2 治本**: 整章鱼完全 transparent 帧 (raw alpha<100 像素数 >28000, 32-laugh 19 帧 / 17-celebrate f16-f21 连续 6 帧). disposal=2 下视觉都是章鱼"挖洞"或"消失". **Pass 1** — per-pixel temporal fill, ±7 滚动窗口 max alpha≥200, 像素 p: curr_a<150 + RGB sum>300 → fill alpha=255. **Pass 2 NEW v9 v2** — silhouette 内 transparent 像素 fill: 检测 alpha<100 像素数 >28000 (整章鱼完全空白) → silhouette = ±5 帧 alpha median≥200 mask; silhouette 内 transparent 像素 → alpha=255 + **RGB = 前一帧 (i-1) RGB** (同 cycle 相邻帧, 时间连续姿势清晰). **不能整帧替换 RGBA** (v9 v1 `frames[i]=frames[i-1].copy()` 触发 PIL APNG encoder cascading bug — disposal=2 下 cascading 帧被错编码成 raw transparent, saved file 报 "APNG contains frame sequence errors", ffmpeg 渲染 saved f012=538 挖洞 vs in-memory f012=17558 opaque. v9 v2 保留 raw transparent 背景 + 仅 fill silhouette, encoder 不触发 cascading 优化, saved n_frames=99 + 视觉正确). **Pass 3** — sub-silhouette temporal median filter, ±5 帧 alpha **中位数** ≥200 → fill. median 比 max 更鲁棒. **Pass 4 NEW v9** — global per-pixel majority voting + **前一帧 RGB fill stable_mask** (取代 v8 的跨帧 median RGB): 跨 99 帧计算每像素 alpha>=200 帧数 ≥70% → stable_mask. fill stable_mask 内 alpha<200 像素到 alpha=255 + RGB = 前一帧 (i-1) 在同位置 RGB. 取消 v7 的 RGB sum>300 守卫 (transparent 帧 RGB 已腐蚀, 守卫让 fill 失败). 前一帧是完整身体帧, RGB 是清晰的姿势色, 不会像跨帧 median 那样把"眯眼/睁眼"混合成折中态. 26 场景共 777390 flicker + 379592 silhouette + 163021 sub-silhouette + 698274 majority-voting 像素修复. **任何走 v10-final 输出的 APNG 上桌前必跑 flicker post-fix v9 v2**.
