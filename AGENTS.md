...
| 项 | 值 |
|---|---|
| 状态 | **V3.0 (2026-09-16) P0 解除**: H3 视频生成 ✅ / 验证 ✅ (18 合格) / mp4 rsync ✅ / v10-final 重生成 18 APNG ✅ (BiRefNet+CorridorKey 治本白方块) / flicker post-fix **v8 three-pass** ✅ (Pass 2 取消治标不治本, Pass 4 跨帧 median RGB fill stable_mask 治本 32-laugh 19 transparent 帧撕裂; 26 场景全套 1042647 majority-voting 像素修复; 章鱼完整粉色 + 姿势自然过渡 + 无挖洞/无半脸/无撕裂) / scenes.json 8→26 ✅ / check-scenes-sync + lint + test ✅. 待 `release-plugin.sh` 跑 V3.0 release. 接手任务见 `HANDOFF.md` |
...
- **v10-final 必跑 post-fix flicker 修复 v8 three-pass** (2026-09-16 加): `scripts/extract-v10-final-postfix-flicker.py`. v10-final 在含大量飞舞道具的场景 (17-celebrate 彩带 / 22-yay-friday 酒杯 / 23-dancing 音符 / 32-laugh 哈哈字 / 36-cheer 彩旗) 上四类 alpha mask 不稳定: (a) 1-frame flicker / (b) 整章鱼空白帧 (CorridorKey 跨帧 unmixing 完全失败) / (c) sub-silhouette 位置性 uncertain / (d) **v8 新覆盖**: 整章鱼完全 transparent 帧 (raw alpha<100 像素数 >28000, 32-laugh 19 帧 / 17-celebrate f16-f21 连续 6 帧). disposal=2 下视觉都是章鱼"挖洞"或"消失". **Pass 1** — per-pixel temporal fill, ±7 滚动窗口 max alpha≥200, 像素 p: curr_a<150 + RGB sum>300 → fill alpha=255. **Pass 2 取消** (v8 治标不治本方向错了): 模板帧 silhouette ≠ 当前帧 silhouette 导致撕裂 (32-laugh f030 左笑右大笑验证). **Pass 3** — sub-silhouette temporal median filter, ±5 帧 alpha **中位数** ≥200 → fill. median 比 max 更鲁棒. **Pass 4 NEW v8 升级** — global per-pixel majority voting + **跨帧 median RGB fill stable_mask** (取消 v7 的 RGB sum>300 守卫): 跨 99 帧计算每像素 alpha>=200 帧数 ≥70% → stable_mask. fill stable_mask 内 alpha<200 像素到 alpha=255 + RGB = 跨 99 帧 median RGB. stable body 区域 (头部/触手) median RGB 稳定 → 章鱼完整; 姿势变化区域 (嘴/眼) median 混合 → 轻微模糊但无撕裂/挖洞. 70% 阈值平衡: 60% fill 姿势切换帧, 80% 覆盖不足. 26 场景共 777390 flicker + 160400 sub-silhouette + 1042647 majority-voting 像素修复. **任何走 v10-final 输出的 APNG 上桌前必跑 flicker post-fix v8**.
