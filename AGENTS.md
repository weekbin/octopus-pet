...
| 项 | 值 |
|---|---|
| 状态 | **V3.0.1 (2026-09-16) released**: 方案 G1 (tmix=frames=2:weights=1 1) 残影修复 4 场景 (17/22/23/32) ✅. 22 场景保持 v9 v2 不动. **GitHub Release v3.0.1**: `octopus-pet.linux.bin` 116MB 含 26 V2 APNG 内嵌 91MB + Rust binary 25MB. 详 `CHANGELOG.md` [Unreleased] "方案 G1 实证成功" + `HANDOFF.md` §5.8. 之前状态: V3.0 (2026-09-16) 26 场景首发 + v9 v2 four-pass 治本 PIL APNG encoder cascading bug.
...
| 方案 G1 (V3.0.1, 2026-09-16 19:13) | **方案 G1 (`ffmpeg tmix=frames=2:weights=1 1`) 实证成功** — 用户接续 G1 选项. 仅帧 i+i-1 平均, **MAD 下降 9.5%** (温和 vs G3 21%). 4 场景覆盖: 32-laugh raw BLANK 24 (vs G3 41) + Pass 2 fill 救回 + very_dark 71762→62541 (-12.9% 改善) + 视觉粉色大笑完整; 23-dancing very_dark 56823→51178 (-9.9%); 17/22 视觉等价 + very_dark 略增 5-7% (可接受). 抽帧对比 + 像素统计全通过. **当前 4 场景 v9 v4 = G1 + 22 场景 v9 v2 = 26 场景 V3.0.1 baseline**. 详 `CHANGELOG.md` [Unreleased] "Added" + `HANDOFF.md` §5.8 |
