...
| **3 V2 APNG** | 50 帧/张 × 132ms ≈ 6.6s 循环, RGBA, 192×192, 2.3MB 各, 走 PIL **v4.2** chroma key (相对绿度 + 双重保护 + alpha 羽化) |
...
| **V2 APNG 生产脚本** | `scripts/extract-chromakey-apng.py` (mp4 → 50 帧 RGBA APNG, v4.2 chroma key 默认, v3 选项兼容) |
...
| **V2 视频 → 桌宠 APNG 流程** | `docs/v2-h3-to-pet-workflow.md` (5 步: H3 双图 → 抽帧 → chroma key v4.2 → alpha 羽化 → APNG, drink-coffee 最新跑通) |
| **V2 抽帧 + chroma key + APNG 一键脚本** | `scripts/extract-chromakey-apng.py` (v4.2 公式沉淀, 14 动作复用) |
...
- **改场景清单 (V2.1 2 场景, 扩到 N+1 个流程不变)**:
  ...
  **加新场景 5 步**:
  1. 跑 `docs/v2-h3-to-pet-workflow.md` (H3 / gen_videos → 抽帧 → chroma key v4.2 → alpha 羽化 → 192×192 APNG)
...
- **V2 视频 → 桌宠 APNG (H3 / gen_videos 走完)**: 走 `docs/v2-h3-to-pet-workflow.md` 完整 5 步 (H3 双图 → ffmpeg 15fps 抽帧 → chroma key v4.2 → alpha 1px Gaussian blur 羽化 → 192×192 APNG). 关键坑:
  - **chroma key v4.2 公式** (`(G - max(R,B)) / G` 相对绿度, `clip((rel - 0.15) / 0.3, 0, 1)`) + **双重保护** (`max(RGB) < 80` 强制不透明保护深色阴影; `80 ≤ max < 150` 且 `G - max(R,B) < 30` 算皮肤保护中绿) + **alpha 羽化** (1 像素 Gaussian blur 让 192→116 resize 边缘从硬切变软边). 详见 `scripts/extract-chromakey-apng.py`.
  ...
  - **chroma key 演进总表** (v1 → v3 → v4 → v4.1 → v4.2): `scripts/extract-chromakey-apng.py` docstring 顶部有完整记录 + 测试集 + 数据验证, 改 chroma key 前必读.
