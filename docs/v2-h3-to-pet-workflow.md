...
> **状态**: ✅ V2.1 production baseline (2026-09-09), `scripts/extract-chromakey-apng.py` 沉淀 v4.7 chroma key 公式. 3 场景 (detective-study / worker-construction / drink-coffee) 已落地桌宠, 14 动作复用同样管线.
>
...
> **chroma key 演进**: v1 → v3 → v4 → v4.1 → v4.2 → v4.3 → v4.4 → v4.5 → v4.5.1 → v4.6 → **v4.7 (当前默认)**, 详见 `scripts/extract-chromakey-apng.py` docstring 顶部.
...
[Step 2] ffmpeg 抽帧 (15fps, PNG 序列)
   ↓
[Step 3] PIL chroma key v4.7 + alpha 激进收紧 (alpha < 80/> 175) + cv2.inpaint 分 3 步 (partial+透明 1px 膨胀 Telea r=8 / green_opaque 6px 膨胀 Telea r=4 / green_tinted_white Telea r=3 修眼白 G 偏色) + 抽帧到 50 帧 + resize 192×192
   ↓
[Step 4] PIL alpha 通道 1px Gaussian blur 羽化 (v4.7 抗锯齿, blur 后低 alpha < 30 归 0)
   ↓
...
**关键参数 (Step 3-5)**: `python3 scripts/extract-chromakey-apng.py --input <mp4> --output <apng>` (默认 chromakey=v4.7, frame-count=50, size=192, duration=132, loop=1).
...
- **alpha 激进收紧** `alpha < 80 → 0, > 175 → 255`: partial 6000 → 4600 (-24-28%), 边缘 hard-key
...
- **v4.7 新增 green_tinted_white mask** (`alpha=255 + R>200 + R+G+B>600 + G>B+5`): 检白色像素 G 偏色, 单独 inpaint Telea r=3 (小半径, 保护眼周星形高光/瞳孔细节) → 治本"眼白发黄/发绿"
...
## Step 3: chroma key v4.7 + cv2.inpaint + 抽帧 + resize
